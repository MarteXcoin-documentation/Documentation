.. meta::
   :description: Guides to secure paper wallets for MarteX cryptocurrency
   :keywords: martex, paper, wallet, encryption

.. _martex-paper-wallet:
...
MarteX, but care must be taken not to lose the private key, since there is
...
highly convenient and user-friendly way to store MarteX long term.
...
guide will use https://martexcoin.org/paperwallet/ as an example. The websites
...
Go to https://martexcoin.org/paperwallet/ in your web browser (or open index.html if
...
the page without somehow recording the martex address and private key, all
...
workflow with a low amount of MarteX before storing significant funds on
...
A MarteX WIF address can be easily identified because it always begins
...
You can send MarteX to a paper wallet address in the same way as to any
other MarteX address. See the documentation for your wallet if you do not
know how to do this. For this example, 0.05 MarteX (minus transaction fee)
...
the wallet into another MarteX wallet that is connected to the internet.
We will use the MarteX Core Wallet in this example, although MarteX Electrum
and mobile wallets are also supported. Open MarteX Core Wallet, click
